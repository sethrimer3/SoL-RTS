...
export function handleMouseUp(e: MouseEvent, state: GameState, canvas: HTMLCanvasElement): void {
  ...
  if (mouseState.selectionRect) {
    ...
    if (selectedBase && state.selectedUnits.size === 0 && !mouseState.touchedBase) {
      ...
      // Handle path drawing or ability drag for selected units
      if (state.settings.movementMode === 'pathDrawing' && mouseState.pathDrawing && mouseState.pathDrawing.rawPath.length > 1) {
        // Execute path drawing movement
        handlePathDrawingEnd(state, mouseState.pathDrawing);
        delete state.pathDrawingPreview;
      } else {
        // Handle ability drag for selected units (tap mode or no path)
        const worldStart = screenToWorldPosition(state, canvas, mouseState.startPos);
